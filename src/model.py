...
"""
模型与训练之间又封装的一个类，
1. 执行模型构建或者加载checkpoint
2. embedding赋值
3. 构造loss 函数，cross_entropy
4. 执行optimizer
5. save模型，load模型
6. 提供三种模式入口：update(train),evaluate,predict
...
class Model:
    ...
    def _update_schedule(self):
        # learning_rate 更新策略
        # 在lr_warmup_steps之内，增加lr
        # 在lr_warmup_steps之后，lr执行指数递减
    ...
    def update(self, batch):
        self.network.train()
        self.opt.zero_grad()    # 清除Tensor里缓存的梯度值
    ...
    def evaluate(self, data):
        ...
        predictions = []
        losses = []
        for batch in data[:self.args.eval_subset]:
            inputs, target = self.process_data(batch)
            with torch.no_grad():
                output = self.network(inputs)
                loss = self.get_loss(output, target)
    ...
    def process_data(self, batch):
        ...
        mask1 = torch.ne(text1, self.args.padding).unsqueeze(2)     # mask初始化
